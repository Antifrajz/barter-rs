use crate::{
    ...
    exchange::simulated::request::SimulatedExchangeRequest,
    order::{
...
pub struct SimulatedExecutionClientConfig {
    pub mocked_exchange: ExchangeId,
    pub request_tx: mpsc::UnboundedSender<SimulatedExchangeRequest>,
    pub event_rx: broadcast::Receiver<UnindexedAccountEvent>,
}

impl Clone for SimulatedExecutionClientConfig {
    fn clone(&self) -> Self {
        Self {
...
impl ExecutionClient for SimulatedExecution {
    ...
    type Config = SimulatedExecutionClientConfig;
    ...
    async fn account_snapshot(
        ...
        self.request_tx
            .send(SimulatedExchangeRequest::fetch_account_snapshot(
                self.time_request(),
                response_tx,
            ))
            .expect("SimulatedExchange is offline - failed to send request");

        let snapshot = response_rx
            .await
            .expect("SimulatedExchange if offline - failed to receive response");
    ...
    async fn cancel_order(
        ...
        self.request_tx
            .send(SimulatedExchangeRequest::cancel_order(
                self.time_request(),
                response_tx,
    ...
    async fn open_order(
        ...
        self.request_tx
            .send(SimulatedExchangeRequest::open_order(
                self.time_request(),
                response_tx,
                into_owned_request(request),
            ))
    ...
    async fn fetch_balances(
        ...
        self.request_tx
            .send(SimulatedExchangeRequest::fetch_balances(
                self.time_request(),
                response_tx,
            ))
            .expect("SimulatedExchange is offline - failed to send request");
    ...
    async fn fetch_open_orders(
        ...
        self.request_tx
            .send(SimulatedExchangeRequest::fetch_orders_open(
                self.time_request(),
                response_tx,
            ))
            .expect("SimulatedExchange is offline - failed to send request");
    ...
    async fn fetch_trades(
        ...
        self.request_tx
            .send(SimulatedExchangeRequest::fetch_trades(
                self.time_request(),
                response_tx,
                time_since,
            ))
